...
- 닉네임 + PIN 4자리 로그인, 방마다가 아니라 **하나의 공유 세계**
- 등급별로 빛나는 무기 그래픽은 **SVG로 코드 생성**(이미지 파일 0개)
...
2. **서버 실행**:
   ...
   node server.js
   ```
3. 터미널에 뜨는 **`같은 네트워크: http://192.168.x.x:3000`** 주소를 동료에게 공유
   - 본인은 `http://localhost:3000` 으로도 접속 가능
